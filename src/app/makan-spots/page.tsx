...
export default function MakanSpotsPage() {
  ...
  const startingPointLat = searchParams.get("starting-point-lat");
  const startingPointLon = searchParams.get("starting-point-lon");
  const destinationLat = searchParams.get("destination-lat");
  const destinationLon = searchParams.get("destination-lon");
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <div className="p-4">
        ...
        {startingPointLat && startingPointLon && destinationLat && destinationLon && (
          <p className="text-sm text-default-500 mt-2">
            Coordinates: Start ({startingPointLat}, {startingPointLon}) → End ({destinationLat}, {destinationLon})
          </p>
        )}
